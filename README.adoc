...
your code, examine the changes, discard the changes and try again rapidly.
...
files. If you're not satisfied, you can simply discard the changes.
...
`discard [PATTERN]`::
Discard changes in the sandbox. This works relative to your current working directory only, and changes outside of your working directory are ignored.
...
`stop [--all]`::
Kill all processes in the sandbox and unmount the filesystems. Note this will not discard any changes
...
files on the host system, or to do things like accept or discard changes in
